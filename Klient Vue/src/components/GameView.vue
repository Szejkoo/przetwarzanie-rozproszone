...
let somebodyWon = ref(
  !!messages?.value.find((el) => el.includes("wygrał grę z liczbą: "))
);
...
const isInputDisabled = () =>
  ...
  gameOverMessage.value.length > 0 ||
  somebodyWon.value;
...
window.addEventListener("beforeunload", () =>
  leaveRoomInvoke(roomName.value, userName.value)
);
</script>
...
<template>
  <div class="flex bg-white">
    <section class="w-full p-4 space-y-2 max-w-3xl mx-auto">
      <ul class="h-[calc(100vh-2rem-.5rem-3rem)] overflow-y-auto text-black">
        <li>Czat:</li>
        <li
          class="p-2 rounded even:bg-emerald-400 bg-white"
      ...
      <form @submit.prevent="handleMakeGuess" class="flex gap-2 h-12">
        ...
        />
        <Button
          class="bg-emerald-400 hover:bg-black"
          type="submit"
    ...
    <aside
      class="max-w-xs w-full bg-emerald-400 p-4 flex flex-col gap-2 h-screen overflow-y-auto"
    >
      <p>Pokój: {{ roomName }}</p>
      ...
      <section>
        ...
        <ul class="list-decimal ml-6">
          <li v-for="user in connectedUsers">
            {{ user }}
            <span v-if="user === userName" class="text-primary font-bold"
              >(Ty)</span
            >
          </li>
        </ul>
