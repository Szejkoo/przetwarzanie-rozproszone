...
const GameView = ({ connectedUsers, messages }: Props) => {
  ...
  const somebodyWon = useMemo(
    () => !!messages.find((el) => el.includes("wygrał grę z liczbą: ")),
    [messages]
  );
  const areControlsDisabled =
    connectedUsers.length < 2 || gameOverMessage.length > 0 || somebodyWon;
  ...
  return (
    <div className="flex">
      <section className="w-full p-4 space-y-2 max-w-3xl mx-auto">
        <ul className="h-[calc(100vh-2rem-.5rem-3rem)] overflow-y-auto">
          ...
          {messages.map((msg, index) => (
            <li
              key={index}
              className="even:bg-secondary p-2 rounded bg-zinc-400"
            >
              {msg}
            </li>
          ))}
      ...
      <aside className="max-w-xs w-full bg-secondary p-4 flex flex-col gap-2 h-screen overflow-y-auto bg-gray-300">
            ...
            {connectedUsers.map((user, index) => (
              <li key={index}>
                ...
                {user === userInfo.userName && (
                  <span className="text-primary font-bold">(Ty)</span>
                )}
              </li>
            ))}
          </ul>
